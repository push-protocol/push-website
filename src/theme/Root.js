...
export default function Root({ children }) {
  ...
  function returnAdditionalClasses(conditions) {
    ...
    for (var i = 0; i < conditions.length; i++) {
      ...
      const pathnameWithBase = baseURL + item?.pathname;
      if (locationPathExists(pathnameWithBase, item.condition)) {
        result = item.classname;
        break;
      }

      const rawPathname = item?.pathname;
      if (
        baseURL &&
        !result &&
        ((item.condition === 'exact' && currentPath === rawPathname) ||
          (item.condition === 'subpaths' &&
            currentPath.startsWith(rawPathname)))
      ) {
        result = item.classname;
        break;
      }
