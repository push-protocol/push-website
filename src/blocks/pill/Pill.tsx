...
const StyledPill = styled.div<PillProps>`
  ...
  background: var(
    --components-pill-background-${({ isActive }) => (isActive ? 'selected' : 'default')}
  );
  cursor: pointer;
  display: flex;
  ...
  ${({ isActive }) =>
    getTextVariantStyles(
      'bs-semibold',
      isActive
        ? 'components-pill-text-selected'
        : 'components-pill-text-default'
    )}
...
const Pill: FC<PillProps> = ({
  ...
  return (
    <StyledPill isActive={isActive} onClick={onClick} {...props}>
      {children}
    </StyledPill>
  );
};
