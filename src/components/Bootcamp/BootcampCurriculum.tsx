...
import { BsCalendar, BsChevronDown, BsChevronUp, BsClock } from "react-icons/bs";
            ...
            {bootcampCurriculum?.map((item, index) => (
                <CurriculumSection key={index}>
                    <CurriculumParent onClick={() => toggleAccordion(index)}>
                        ...
                        <div>
                            {activeIndex === index ? 
                                <BsChevronDown color="#FFF" size={22} /> : <BsChevronUp color="#FFF" size={22} />}
                        </div>
                    </CurriculumParent>
                    
                    {activeIndex === index && (
                        <ItemH flexDirection={isMobile && 'column'} padding="0 0 24px 0" gap="32px">
