...
function EPNSChannelItem({ channelID, loaded, epnsContract }) {
  ...
  React.useEffect(() => {
    ...
    if (loaded && channelID && epnsContract) {
      ...
      EPNSHelperAPI.getChannelJsonFromChannelAddress(
        channelID,
        epnsContract
      )
        .then((channelMeta) => {
        ...
        .catch((err) => {
          if (enableLogs)
            console.log(
              "!!!Error, getChannelMetaAsync() inside ChannelJSON --> %o",
              err
            );
          throw err;
        });
        ...
        <ChannelLogo>
          <ChannelLogoOuter>
            <ChannelLogoInner>
              ...
              {!loading && <Image src={`${meta.icon}`} />}
            </ChannelLogoInner>
          </ChannelLogoOuter>
        </ChannelLogo>

        <Item self="stretch" align="center" margin="10px 0px 0px 0px">
          {loading && <Skeleton color="#eee" width={"80%"} height={24} />}
          {!loading && <Span textAlign="center">{meta.name}</Span>}
        </Item>
      </Anchor>
    </ChannelItem>
  );
