...
Unlike emails, chat messages are typically shorter and more frequent, but the core idea—serializing your content—remains the same. Let’s wrap our text, sender info, and timestamp in JSON:
...
const serializedChat = new TextEncoder().encode(JSON.stringify(chatMessage));
```
...
transactionResponse.blocks.forEach((block: SimplifiedBlockType) => {
  block.transactions.forEach((t: SimplifiedTxResponse) => {
    const decodedData = new TextDecoder().decode(
      new Uint8Array(Buffer.from(t.txnData as unknown as string, 'hex'))
    );
    const chatTxData = JSON.parse(decodedData) as {
      sender: string;
      text: string;
      timestamp: number;
    };
    console.log(chatTxData);
  });
});
```
...
async function main() {
  ...
  const transactionResponse = (await pushChain.tx.get(tx.txHash)) as SimplifiedBlockResponse;
