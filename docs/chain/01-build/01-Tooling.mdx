...
![Metamask PushChain Configuration](../images/Metamask-Push-Chain.png)
...
// 1. Define Push Chain as a custom chain
...
// 2. Initialize a PublicClient using the HTTP transport
const client = createPublicClient({
  chain: pushChain,
  transport: http(),
});

// 3. Fetch the latest block
async function getLatestBlock() {
  const block = await client.getBlock({ blockNumber: "latest" });
  console.log("Latest block:", block);
}

// 4. Fetch a transaction by its hash
