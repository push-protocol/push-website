...
The `pushChain.block.get` function allows developers to fetch blocks by their hash or by filtering by time.
...
| `startTime`      | `number` (timestamp)                     | Fetches blocks starting from this timestamp.                                                | Current timestamp  |
| `order`          | `Order` (`'ASC'` or `'DESC'`)            | Determines the sort order of blocks (`'ASC'` for ascending, `'DESC'` for descending).        | `'DESC'`           |
...
Here below is an example of how to fetch a block by its hash.
