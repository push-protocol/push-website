...
| **Param**          | **Type**                                                                                                                          | **Remarks**                                                         |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| `universalAccount` | `UniversalAccount`                                                                                                                | A UniversalAccount object containing chain and address information. |
| `signMessage`      | `(data: Uint8Array) => Promise<Uint8Array>`                                                                                       | A function to sign a message.                                       |
| `signTransaction`  | `(unsignedTx: Uint8Array) => Promise<Uint8Array>`                                                                                 | A function to sign a transaction.                                   |
| `signTypedData`    | `(data: { domain: TypedDataDomain, types: TypedData, primaryType: string, message: Record<string, any> }) => Promise<Uint8Array>` | A function to sign a typed data.                                    |
...
const signMessage = async (data: Uint8Array) => {
const hexSig = await viemAccount.signMessage({
message: { raw: data },
})
return hexToBytes(hexSig)
}
const signTransaction = async (unsignedTx: Uint8Array) => {
const tx = parseTransaction(bytesToHex(unsignedTx))
const txHash = await viemAccount.signTransaction(tx as never)
