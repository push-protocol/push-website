...
<head>
  <title>
    Managa Subscriptions | Push Notification | Push Documentation
  </title>
</head>
...
const response = await userAlice.notification.subscribe(`eip155:5:${channelAddress}`, {
  settings: [ // settings are dependent on channel
    setting1: {enabled: true, value: '1'},
    setting2: {enabled: false},
    setting3: {enabled: true, value: '50'}
  ]
})
```
...
| ----------- | ------------------- | ---------------------- | ------- | ----------------------------------------------------------------------- |
| _`channel`_ | `string`            | -                      | -       | Channel/Alias address in [chain specific wallet format](/docs/notifications/build/important-concepts/#chain-specific-wallet-address) |
