...
sidebar_position: 14
image: "/static/assets/docs/previews/docs_notifications_develop--stream_notifications.png"
---
...
const stream = userAlice.stream(account?, { 
  ...
  // more info can be found at push.org/docs/chat
	listen: [STREAM.CONNECT, STREAM.NOTIF, STREAM.DISCONNECT], 
	filter?: {
...
userAlice.stream.on(STREAM.CONNECT, (data: any) => {
  console.log('stream is connected')
})

// Listen for stream disconnection 
userAlice.stream.on(STREAM.DISCONNECT, (data: any) => {
