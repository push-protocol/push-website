...
## Reinstate Notification stream

To reinstate a Notification stream. Useful when you want to listen to more events or change the filter options.

<Tabs className="codetabs" groupId="code-examples">
  <TabItem value="js" attributes={{className: "codetab js"}} default>

```typescript
// say you created a stream like this, const stream = await userAlice.initStream( [ 'STREAM.VIDEO', 'STREAM.CONNECT', 'STREAM.DISCONNECT' ]);
...
```typescript
// say you created a stream like above, const stream = await userAlice.initStream( [ 'STREAM.NOTIF', 'STREAM.CONNECT', 'STREAM.DISCONNECT' ]);
