...
```typescript
// userAlice.channel.notifications({options?})
// Default, it fetches all the notifications from userAlice's Channel
const allNotifications = await userAlice.channel.notifications();
```
...
| --------- | ----------------- | --------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `options` | `object`          | -         | -       | An object containing additional options for filtering and pagination.                                                                                                                                                                                                   |
| -         | `options.account` | `string`  | -       | Channel address that you want fetch notifications for, [Account supports a number of address format](/docs/notifications/important-concepts/#types-of-supported-wallet-address-account) including wallet addresses, chain agnostic wallet addresses, NFT addresses, etc |
| -         | `options.page`    | `number`  | 1       | A number representing the page of results to retrieve.                                                                                                                                                                                                                  |
| -         | `options.limit`   | `number`  | 10      | A number representing the maximum number of feeds to retrieve per page.                                                                                                                                                                                                 |
| -         | `options.raw`     | `boolean` | False   | A boolean indicating whether to retrieve raw feed data.                                                                                                                                                                                                                 |
| -         | `options.filter`  | `string`  | -       | Filter the notifications by their type. `1` for broadcast notifcations and likewise, Refer [Notification Types Section](/docs/notifications/build/types-of-notification) to learn more.                                                                                 |
