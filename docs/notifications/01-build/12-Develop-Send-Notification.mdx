...
| -              | `options.payload.category`       | `string`                                          | -       | Notification category setting index, if present, the user prefernce (if they have opted for that setting) will determine if notification is delivered to them or not    |
