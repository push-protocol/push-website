...
<head>
  <title>Integrating Notifications | Push Notifications | Push Documentation</title>
</head>
...
function App(props) {
  ...
  const triggerNotification = async () => {
    ...
    // stream connected
    userAlice.stream.on(STREAM.CONNECT, (data) => {
...
<ABlock href="/docs/notifications/tutorials/integrate-notifications-via-delivery-node" title="Tutorial on how to implement bridge notification delivery mechanism for Push protocol using Push delivery node">Setup and bridge notifications via Push Delivery Node</ABlock>
