...
You can access the [code](https://github.com/ethereum-push-notification-service/push-showrunners-framework-staging/tree/main/src/sample_showrunners/aave) for the channel and get some vibe for how the code is looking !!

<head>
  <title>DeFi based trigger | Example | Showrunners Scaffold | Push Notifications |
...
export default () => {
  ...
  logger.info(
    `     🛵 Scheduling Showrunner - Aave Channel [on 6 Hours] [${new Date(
      Date.now(),
    )}]`,
  );
