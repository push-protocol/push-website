...
if (userSettings[9]?.enabled == true && userSettings[10]?.enabled == true) {
    ...
    if (Math.abs(Number(changePercentage)) >= userValuePrice && userDBValue.lastCycle + userValueTime == CYCLES) {
