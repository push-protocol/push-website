...
```typescript
 ...
 // If the received status is DenyVideo that means the peer has denied the call
  if(data.event === VideoEventType.DenyVideo){
    // here you can update the UI with a toast or something that the peer denied the call
  }
