...
<head>
  <title>Manage User | Push Video | Push Documentation</title>
</head>
...
| -         | `options.overrideAccount` | -        | -       | The [account](/docs/video/supported-wallet-standards 'Push Video support wallet standards') for which info is retrieved, can override to get info of other accounts not owned by the user. If not provided, it is derived from signer. |
...
:::tip
Attaching progress hook functionality in [Initialize user API](/docs/video/build/initialize-user) will help you to track the progress of encryption update.
:::
