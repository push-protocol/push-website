...
```js
  ...
  // store the incoming peer address
  let incomingCallerAddress: string;
  ...
  stream.on(CONSTANTS.STREAM.VIDEO, async (data : TYPES.VIDEO.EVENT) => {
    if (data.event === CONSTANTS.STREAM.VIDEO.REQUEST) {
      ...
      // optionally store the incoming peer address to handle multiple call requests 
      // you will be passing the stored caller address in approve()/deny() method
      incomingCallerAddress = data.peerInfo.address
    }
