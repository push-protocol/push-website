...
When a user receives an incoming video call request, it needs to be approved by the user before the call can be established. To approve an incoming video call request, we call the `approve()` method and pass the received `incomingCallerAddress` data from the stream event.

```typescript
// aliceVideoCall.approve(address);
await aliceVideoCall.approve(incomingCallerAddress);
```

| Property    | Type     | Description                                                                |
| ----------- | -------- | -------------------------------------------------------------------------- |
| _`address`_ | `string` | `address` of the caller received from the stream event in the Push Stream. |
...
When a user receives an incoming video call request, it can be denied/rejected by the user. To deny/reject an incoming video call request, we call the `deny()` method and pass the received `incomingCallerAddress` data from the stream.

```typescript
// aliceVideoCall.deny(address);
await aliceVideoCall.deny(incomingCallerAddress);
```
