...
| -            | `options.rules`                  | `any[]`    | -       | Define conditions such as token gating, nft gating, custom endpoint for `joining` or `sending message` in a group. See [conditional group gating](./conditional-rules-for-group) to understand rule engine and how to fine tune conditional rules of your group |
...
Learn about <AFocus href='./conditional-rules-for-group'>conditional rules</AFocus> to understand how groups permissions such as `joining group` or `sending messages` can be customized for your community or needs.
