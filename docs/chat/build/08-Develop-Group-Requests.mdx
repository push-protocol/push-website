...
_`Important`_ Some groups might be gated and user might not be permitted to do certain actions, such as `sending message` or event `joining group`. You can find out if a group is gated by calling [chat.group.permissions API](./group-permissions)
