...
<head>
  <title>
    MessageInput | UI Components (@pushprotocol/uiweb) | Push Chat | Push
    Documentation
  </title>
</head>
...
### Push MessageInput Component live playground
```jsx live 
...
function App(props) {
  return (
    <>
      <h2>Live chat with pushai.eth, connect your wallet and start sending messages. You can customise the input field by changing the paramters</h2>
        <ChatUIProvider>
          <MessageInput chatId="0x99A08ac6254dcf7ccc37CeC662aeba8eFA666666" emoji={true} gif={true} file={true} />
        </ChatUIProvider>
    </>
  );
}
