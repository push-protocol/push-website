---
id: chat-quickstart
title: Quickstart
hide_title: false
slug: ./quickstart
displayed_sidebar: pushChatSidebar
sidebar_position: 2
---

# Quickstart

Everything you will need to get up and running in 2 mins or less!

<head>
  <title>Push Chat Quickstart | Push Documentation</title>
</head>

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

### Installation

<Tabs className="codetabs" groupId="code-examples">
  <TabItem value="js" attributes={{className: "codetab js"}} default>
    

  ```js
  // Install Libraries
  npm install @pushprotocol/restapi@latest @pushprotocol/socket@latest ethers@^5.6
  ```

  </TabItem>
  <TabItem value="react" attributes={{className: "codetab react"}} default>
    

  ```js
  // Install Libraries
  npm install @pushprotocol/restapi@latest @pushprotocol/socket@latest ethers@^5.6
  ```

  </TabItem>
  <TabItem value="reactnative" attributes={{className: "codetab reactnative"}} default>
    

  ```js
  // Install Libraries
  npm install @pushprotocol/restapi@latest @pushprotocol/socket@latest ethers@^5.6
  ```

  </TabItem>
</Tabs>


### Import Libraries

<Tabs className="codetabs" groupId="code-examples">
  <TabItem value="js" attributes={{className: "codetab js"}} default>
    

  ```js
  // Import restapi for function calls  
  // Import socket for listening for real time messages
  import * as PushAPI from '@pushprotocol/restapi';
  import { createSocketConnection, EVENTS } from '@pushprotocol/socket';

  // Ethers v5 or Viem, both are supported
  import { ethers } from 'ethers';
  ```

  </TabItem>

  <TabItem value="react" attributes={{className: "codetab react"}} default>
    

  ```js
  // Import restapi for function calls  
  // Import socket for listening for real time messages
  import * as PushAPI from '@pushprotocol/restapi';
  import { createSocketConnection, EVENTS } from '@pushprotocol/socket';

  // Ethers v5 or Viem, both are supported
  import { ethers } from 'ethers';
  ```

  </TabItem>

  <TabItem value="reactnative" attributes={{className: "codetab reactnative"}} default>
    

  ```js
  // Import restapi for function calls  
  // Import socket for listening for real time messages
  import * as PushAPI from '@pushprotocol/restapi';
  import { createSocketConnection, EVENTS } from '@pushprotocol/socket';

  // Ethers v5 or Viem, both are supported
  import { ethers } from 'ethers';
  ```

  </TabItem>
</Tabs>

### Initialize Chat

<Tabs className="codetabs" groupId="code-examples">
  <TabItem value="js" attributes={{className: "codetab js"}} default>
    

  ```js
  // Creating a random signer from a wallet, ideally this is the wallet you will connect
  const signer = ethers.Wallet.createRandom();

  // Initialize wallet user, pass 'prod' instead of 'staging' for mainnet apps
  const userAlice = await PushAPI.initialize(signer, { env: 'staging' });
  ```

  </TabItem>

  <TabItem value="react" attributes={{className: "codetab react"}} default>
    

  ```js
  // Creating a random signer from a wallet, ideally this is the wallet you will connect
  const signer = ethers.Wallet.createRandom();

  // Initialize wallet user, pass 'prod' instead of 'staging' for mainnet apps
  const userAlice = await PushAPI.initialize(signer, { env: 'staging' });
  ```

  </TabItem>

  <TabItem value="reactnative" attributes={{className: "codetab reactnative"}} default>
    

  ```js
  // Creating a random signer from a wallet, ideally this is the wallet you will connect
  const signer = ethers.Wallet.createRandom();

  // Initialize wallet user, pass 'prod' instead of 'staging' for mainnet apps
  const userAlice = await PushAPI.initialize(signer, { env: 'staging' });
  ```

  </TabItem>
</Tabs>

### Send Message

<Tabs className="codetabs" groupId="code-examples">
  <TabItem value="js" attributes={{className: "codetab js"}} default>
    

  ```js
    // This will be the wallet address of the recipient 
    const toWalletAddress = ethers.Wallet.createRandom().address;

    // Send a message to Bob
    const aliceMessagesBob = await userAlice.chat.send(toWalletAddress, {
      content: 'Hello Bob!',
      type: 'Text',
    });
  ```

  </TabItem>

  <TabItem value="react" attributes={{className: "codetab react"}} default>
    

  ```js
    // This will be the wallet address of the recipient 
    const toWalletAddress = ethers.Wallet.createRandom().address;

    // Send a message to Bob
    const aliceMessagesBob = await userAlice.chat.send(toWalletAddress, {
      content: 'Hello Bob!',
      type: 'Text',
    });
  ```

  </TabItem>

  <TabItem value="reactnative" attributes={{className: "codetab reactnative"}} default>
    

  ```js
    // This will be the wallet address of the recipient 
    const toWalletAddress = ethers.Wallet.createRandom().address;

    // Send a message to Bob
    const aliceMessagesBob = await userAlice.chat.send(toWalletAddress, {
      content: 'Hello Bob!',
      type: 'Text',
    });
  ```

  </TabItem>
</Tabs>

### Stream / Real time updates / Socket

<Tabs className="codetabs" groupId="code-examples">
  <TabItem value="js" attributes={{className: "codetab js"}} default>
    

  ```js
  // Create Socket Connection
  const pushSDKSocket = createSocketConnection({
    user: signer.wallet,
    socketType: 'chat',
    socketOptions: { autoConnect: true, reconnectionAttempts: 3 },
    env: 'staging',
  });

  // EVENTS.CHAT_RECEIVED_MESSAGE - To listen to real time message
  pushSDKSocket.on(EVENTS.CHAT_RECEIVED_MESSAGE, (message) => {
    // Register EVENTS.CHAT_RECEIVED_MESSAGE to recieve chat messages and new requests
    console.log(message);
  });
  ```

  </TabItem>

  <TabItem value="react" attributes={{className: "codetab react"}} default>
    

  ```js
  // Create Socket Connection
  const pushSDKSocket = createSocketConnection({
    user: signer.wallet,
    socketType: 'chat',
    socketOptions: { autoConnect: true, reconnectionAttempts: 3 },
    env: 'staging',
  });

  // EVENTS.CHAT_RECEIVED_MESSAGE - To listen to real time message
  pushSDKSocket.on(EVENTS.CHAT_RECEIVED_MESSAGE, (message) => {
    // Register EVENTS.CHAT_RECEIVED_MESSAGE to recieve chat messages and new requests
    console.log(message);
  });
  ```

  </TabItem>

  
  <TabItem value="reactnative" attributes={{className: "codetab reactnative"}} default>
    

  ```js
  // Create Socket Connection
  const pushSDKSocket = createSocketConnection({
    user: signer.wallet,
    socketType: 'chat',
    socketOptions: { autoConnect: true, reconnectionAttempts: 3 },
    env: 'staging',
  });

  // EVENTS.CHAT_RECEIVED_MESSAGE - To listen to real time message
  pushSDKSocket.on(EVENTS.CHAT_RECEIVED_MESSAGE, (message) => {
    // Register EVENTS.CHAT_RECEIVED_MESSAGE to recieve chat messages and new requests
    console.log(message);
  });
  ```

  </TabItem>
</Tabs>