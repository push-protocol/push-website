---
id: devs-chat-develop-conditional-rules-for-group
...
- Either of criteria should be able to allow user to join the group, this means that the **operator** to use to combine these criterias would be `any`

```typescript
// Operator object - 'any' since either condition should allow access
any: [
  {
    type: "PUSH", // define type that rules engine should go for
    category: "ERC20", // define it's ERC20 token that you want to check, supports ERC721 as well
...
- Finally pass this rules object into the group that we are creating using the [create group API call](chat.group.create).
```typescript
